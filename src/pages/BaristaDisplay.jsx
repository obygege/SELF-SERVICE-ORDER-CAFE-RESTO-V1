...
const BaristaDisplay = () => {
    ...
    useEffect(() => {
        ...
        const unsub = onSnapshot(q, (snapshot) => {
            ...
            if (newest && newest.id !== lastProcessedId) {
                ...
                toast(`Pesanan Minuman Baru: Meja ${newest.tableNumber}`, {
                    icon: '☕',
                    style: { borderRadius: '15px', background: '#0369a1', color: '#fff', fontWeight: 'bold' }
                });
            }
        });
        return () => unsub();
    ...
    const updateStatus = async (id, newStatus) => {
        try {
            ...
            toast.success(newStatus === 'cooking' ? 'Sedang Diracik...' : 'Minuman Selesai!');
        } catch (error) {
    ...
    const handleLogout = async () => {
        if (window.confirm("Keluar dari sistem Barista?")) await logout();
    };
                        ...
                        <div key={order.id} className={`flex flex-col rounded-[2.5rem] bg-white border-2 transition-all duration-500 shadow-sm ${isReady ? 'border-green-500 ring-4 ring-green-50' : isCooking ? 'border-blue-500 scale-[1.02] shadow-xl z-10' : 'border-slate-100'}`}>
                            ...
                            <div className="p-6 flex-1 space-y-4">
                                {myItems.map((item, idx) => (
                                    <div key={idx} className="flex gap-4 items-start border-b border-slate-50 pb-3 last:border-0">
                                        <div className="w-10 h-10 shrink-0 flex items-center justify-center bg-blue-50 rounded-xl font-black text-xl text-blue-900">
                                            {item.qty}
                                        </div>
                            ...
                            <div className="p-6 pt-0 space-y-3">
                                {order.status === 'pending' && (
                                    <button onClick={() => updateStatus(order.id, 'cooking')} className="w-full py-5 bg-slate-900 hover:bg-blue-600 text-white font-black text-xs uppercase tracking-[0.2em] rounded-2xl transition-all shadow-lg active:scale-95 flex items-center justify-center gap-3">
