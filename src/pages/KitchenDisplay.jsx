    ...
    return (
        <div ref={ref} className="bg-white text-black font-mono p-4" style={{ width: '58mm', fontSize: '12px' }}>
            ...
            <div className="text-center border-b-2 border-black pb-2 mb-2">
                <h2 className="font-black text-xl">DAPUR (FOOD)</h2>
...
const KitchenDisplay = () => {
        ...
        const unsub = onSnapshot(q, (snapshot) => {
            ...
            if (newest && newest.id !== lastProcessedId) {
                ...
                if (hasMyItems) {
                    ...
                    toast(`Order Makanan Baru Meja ${newest.tableNumber}!`, { icon: '🔔', style: { borderRadius: '10px', background: '#e65100', color: '#fff' } });
                }
            }
        });
        return () => unsub();
    ...
    const updateStatus = async (id, status) => {
        ...
        toast.success(status === 'cooking' ? 'Sedang Dimasak...' : 'Makanan Siap Diantar!');
    };

    const handleLogout = async () => {
        if (window.confirm("Keluar dari Kitchen Monitor?")) {
            await logout();
        }
    };
        ...
        <div className="min-h-screen bg-gray-50 text-gray-800 font-sans p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {orders.map(order => {
                    ...
                    return (
                        <div key={order.id} className={`flex flex-col rounded-xl overflow-hidden shadow-lg border-2 transition-all duration-300 ${isReady ? 'border-green-500 bg-green-50' : isCooking ? 'border-orange-400 bg-white scale-105 z-10 shadow-xl' : 'border-gray-200 bg-white'}`}>

                            <div className={`p-4 flex justify-between items-center ${isReady ? 'bg-green-100 text-green-800' : isCooking ? 'bg-orange-100 text-orange-800' : 'bg-gray-100 text-gray-600'}`}>
                                <div>
                                ...
                                <div className="text-right">
                                    ...
                                    <div className="text-[10px] font-black uppercase mt-1">
                                        {isReady ? 'SIAP SAJI' : isCooking ? 'DIMASAK' : 'ANTRIAN'}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className={`p-4 flex-1 space-y-4 overflow-y-auto max-h-[300px] ${isReady ? 'bg-green-50' : 'bg-white'}`}>
                                {myItems.map((item, idx) => (
                                    <div key={idx} className="flex gap-3 items-start border-b border-gray-100 pb-3 last:border-0">
                                        <div className={`w-10 h-10 flex items-center justify-center rounded-lg font-black text-xl shadow-sm border ${isReady ? 'bg-green-200 border-green-300 text-green-800' : isCooking ? 'bg-orange-50 border-orange-200 text-orange-700' : 'bg-gray-50 border-gray-200 text-gray-700'}`}>
                                            {item.qty}
                                        </div>
                            ...
                            <div className="p-3 bg-gray-50 border-t border-gray-100 grid gap-2">
                                {order.status === 'queue' && (
                                    <button onClick={() => updateStatus(order.id, 'cooking')} className="w-full py-4 bg-gray-800 hover:bg-orange-600 text-white font-bold text-xl rounded-lg transition-colors flex items-center justify-center gap-2 shadow-lg shadow-gray-200">
                                        <PlayCircle size={24} /> MULAI MASAK
                                    </button>
                                )}
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm flex flex-col items-center animate-in zoom-in duration-200">
                        <div className="flex justify-between items-center w-full mb-4 border-b pb-2">
                            <h3 className="font-bold text-lg text-gray-800 flex items-center gap-2"><Printer size={20} /> Preview Struk Dapur</h3>
                        ...
                        <div className="grid grid-cols-2 gap-3 w-full">
                            ...
                            <button onClick={() => setPrintingOrder(previewOrder)} className="py-3 rounded-lg bg-orange-600 text-white font-bold hover:bg-orange-700 shadow-lg flex items-center justify-center gap-2">
