...
const AdminLayout = () => {
    ...
    const isActive = (path) => {
        if (path === '/head' && location.pathname === '/head') return 'text-orange-600';
        if (path !== '/head' && location.pathname.startsWith(path)) return 'text-orange-600';
        ...
        <div className="flex h-screen bg-gray-100 font-sans overflow-hidden">
            ...
            <main className="flex-1 overflow-y-auto bg-gray-50 relative pb-24 md:pb-0">
                <header className="bg-white/80 backdrop-blur-md sticky top-0 z-10 border-b px-4 md:px-8 py-4 flex justify-between items-center">
                    ...
                    <div className="h-9 w-9 bg-gradient-to-tr from-orange-500 to-red-500 rounded-full flex items-center justify-center text-white font-bold shadow-md text-sm">
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 flex justify-around items-center py-3 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                {userRole === 'admin' ? (
                    <>
                        <Link to="/admin/live-orders" className={`flex flex-col items-center gap-1 ${isActive('/admin/live-orders')}`}>
                            <ClipboardList size={20} /> <span className="text-[9px] font-bold">Pesanan</span>
                        </Link>
                        <Link to="/admin/history" className={`flex flex-col items-center gap-1 ${isActive('/admin/history')}`}>
                            <History size={20} /> <span className="text-[9px] font-bold">Riwayat</span>
                        </Link>
                        <Link to="/admin/products" className={`flex flex-col items-center gap-1 ${isActive('/admin/products')}`}>
                            <Coffee size={20} /> <span className="text-[9px] font-bold">Menu</span>
                        </Link>
                        <Link to="/admin/settings" className={`flex flex-col items-center gap-1 ${isActive('/admin/settings')}`}>
                            <Settings size={20} /> <span className="text-[9px] font-bold">Setting</span>
                        </Link>
                    ...
                    <>
                        <Link to="/head" className={`flex flex-col items-center gap-1 ${isActive('/head')}`}>
                            <FileBarChart size={20} /> <span className="text-[10px] font-bold">Laporan</span>
                        </Link>
                        <Link to="/profile" className={`flex flex-col items-center gap-1 ${isActive('/profile')}`}>
